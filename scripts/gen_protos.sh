...
for f in ${OUT_DIR}/*_pb2.py ${OUT_DIR}/*_pb2_grpc.py; do
  if [ -f "$f" ]; then
    # If file already contains the package import, skip
    if grep -q "from proto import .*_pb2 as" "$f"; then
      continue
    fi
    # Replace patterns like: import control_pb2 as control__pb2
    sed -i "s/^import \(.*_pb2\) as \(.*\)$/from proto import \1 as \2/" "$f" || true
    # Replace relative imports generated by some protoc variants: from . import control_pb2 as control__pb2
    sed -i "s/^from \. import \(.*_pb2\) as \(.*\)$/from proto import \1 as \2/" "$f" || true
  fi
done
